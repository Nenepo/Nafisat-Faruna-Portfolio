...
function Header() {
  ...
  const toggleNavigation = () => {
    ...
    gsap.to("#text", {
      y:openNavigation ? 0 : '20%', duration: 1, ease: 'power2.inOut', delay: 0.2
    })
        ...
        <TextAnimation openNavigation={openNavigation} />
          <nav id="intro" className=" mx-auto md:mx-0 md:flex md:gap-4  ">
           ...
           <div className="parent overflow-hidden" key={index}>
              <a
              // transitiion from to x
              // starts from the center and expands / how can i make the border tiny and small? should i juts use another div?
              key={item.id}
                href={item.url}
                onClick={() =>setOpenNavigation(false)
                
             }
                className={`line relative uppercase block text-[3rem] font-[500] md:text-lg font-poppins text-n-1 transition tracking-tighter border-opacity-0 
